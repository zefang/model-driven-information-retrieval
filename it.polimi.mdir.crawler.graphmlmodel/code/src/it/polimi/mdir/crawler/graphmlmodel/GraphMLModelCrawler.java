...
import it.polimi.mdir.crawler.graphmlmodel.messages.Attribute;
import it.polimi.mdir.crawler.graphmlmodel.messages.Process.Filter;
import it.polimi.mdir.crawler.graphmlmodel.messages.Process.Filter.Exclude;
import it.polimi.mdir.crawler.graphmlmodel.messages.Process.Filter.Include;
...
public class GraphMLModelCrawler extends AbstractCrawler {
  ...
  public void initialize(final DataSourceConnectionConfig config) throws CrawlerException, CrawlerCriticalException {
    ...
    _performanceCounters =
      new CrawlerPerformanceCounterHelper<GraphMLModelCrawlerPerformanceAgent>(config, hashCode(),
        GraphMLModelCrawlerPerformanceAgent.class);
  ...
  private void checkFolders(final DataSourceConnectionConfig config) throws CrawlerCriticalException {
    final it.polimi.mdir.crawler.graphmlmodel.messages.Process process = (it.polimi.mdir.crawler.graphmlmodel.messages.Process) config.getProcess();
  ...
  private Serializable readAttribute(final File file, final Attribute attribute, final boolean forceByteToString)
    ...
    switch (attribute.getFileAttributes()) {
      ...
      case PROJECT_ID:
    	xq = new XQueryWrapper("../it.polimi.mdir.crawler.graphmlmodel/xquery/getProjectId.xquery");
      ...
      case CLASS_NAMES:
    	xq = new XQueryWrapper("../it.polimi.mdir.crawler.graphmlmodel/xquery/getClassNames.xquery");
        xq.bindVariable("document", file.getAbsolutePath());
        resultList = xq.executeQuery();
        resultListString = arrayListToString(resultList);
        return resultListString; 
      case CLASS_IDS:
    	xq = new XQueryWrapper("../it.polimi.mdir.crawler.graphmlmodel/xquery/getClassIds.xquery");
        xq.bindVariable("document", file.getAbsolutePath());
        resultList = xq.executeQuery();
        resultListString = arrayListToString(resultList);
        return resultListString;
      ...
      case ATTRIBUTE_NAMES:
    	xq = new XQueryWrapper("../it.polimi.mdir.crawler.graphmlmodel/xquery/getNodeAttributes.xquery");
        xq.bindVariable("document", file.getAbsolutePath());
        resultList = xq.executeQuery();
        resultListString = arrayListToString(resultList);
        
        xq = new XQueryWrapper("../it.polimi.mdir.crawler.graphmlmodel/xquery/getEdgeAttributes.xquery");
        xq.bindVariable("document", file.getAbsolutePath());
        resultList = xq.executeQuery();
  ...
  */
  ...
  private class CrawlingProducerThread extends Thread {
     ...
     * The _process.
     */
    private final it.polimi.mdir.crawler.graphmlmodel.messages.Process _process;
    ...
    public CrawlingProducerThread(final CrawlerCallback crawlerCallback,
      ...
      _process = (it.polimi.mdir.crawler.graphmlmodel.messages.Process) process;
