...
public class LuceneSchemaResolver implements SchemaResolver, ILuceneSchemaResolver {
  ...
  private static final String[] KNOWN_SCHEMAS = { "IndexStructure.xsd", "DataDictionaryConnection.xsd",
    "AnyFinderSearchDateFieldParameter.xsd", "AnyFinderSearchNumberFieldParameter.xsd",
    "AnyFinderSearchTextFieldParameter.xsd", "AnyFinderAdvancedSearch.xsd", };
  ...
  public byte[] getSchemaByName(final String schemaName) {
    ...
    final File folder = ConfigUtils.getConfigFolder(LuceneService.BUNDLE_NAME, "xml");
    if (folder == null) {
      if (log.isErrorEnabled()) {
        log.error("unable to locate configuration folder [" + LuceneService.BUNDLE_NAME + "/xml]");
      }
      return null;
    }
