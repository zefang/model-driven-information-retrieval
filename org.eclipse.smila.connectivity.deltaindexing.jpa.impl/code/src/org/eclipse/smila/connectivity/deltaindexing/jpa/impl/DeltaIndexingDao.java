 ...
 */
// @NamedQueries( {
// @NamedQuery(name = "DeltaIndexingDao.findSubCompounds", query =
// "SELECT d FROM DeltaIndexingDao d WHERE d._parentIdHash = :parentIdHash AND d._isCompound = 1"),
// @NamedQuery(name = "DeltaIndexingDao.findObsoleteIdsBySource", query =
// "SELECT d FROM DeltaIndexingDao d WHERE d._dataSourceId = :source AND d._visited = 0 AND (d._parentIdHash IS NULL OR d._parentIdHash IN (SELECT dd._idHash FROM DeltaIndexingDao dd WHERE dd._isCompound = 1 AND (dd._visited = 0 OR dd._modified = 1)))"),
// @NamedQuery(name = "DeltaIndexingDao.deleteBySource", query =
// "DELETE FROM DeltaIndexingDao d WHERE d._dataSourceId = :source"),
// @NamedQuery(name = "DeltaIndexingDao.deleteAll", query = "DELETE FROM DeltaIndexingDao"),
// @NamedQuery(name = "DeltaIndexingDao.resetFlags", query =
// "UPDATE DeltaIndexingDao d SET d._visited = 0, d._modified = 0 WHERE d._dataSourceId = :source"),
// @NamedQuery(name = "DeltaIndexingDao.countEntries", query =
// "SELECT d._dataSourceId, COUNT(d._dataSourceId) FROM DeltaIndexingDao d GROUP BY d._dataSourceId"),
// @NamedQuery(name = "DeltaIndexingDao.countEntriesBySource", query =
// "SELECT COUNT(d) FROM DeltaIndexingDao d WHERE d._dataSourceId = :source") })
public class DeltaIndexingDao implements Serializable {
   ...
   *           if any error occurs
   */
  public DeltaIndexingDao(final ConnectivityId id, final String hash, final boolean isCompound,
    ...
    _hash = hash;
    _dataSourceId = id.getDataSourceId();
    _isCompound = isCompound;
    _visited = visited;

    final ConnectivityId parentId = id.getContainerId();
    if (parentId != null) {
   ...
   *           if any exception occurs
   */
  public ConnectivityId toId() throws IOException, ClassNotFoundException {
    final ObjectInputStream objectStream = new ObjectInputStream(new ByteArrayInputStream(_serializedId));
    return (ConnectivityId) objectStream.readObject();
  }
   ...
   */
  public void modifyAndVisit(final String hash) {
    _visited = true;
    _modified = true;
    _hash = hash;
  }
