...
public class TestDlq extends DeclarativeServiceTestCase {
  ...
  private void executeDLQCauseCycle(final String dataSourceID) throws RouterException, InterruptedException,
    JMSException, DeserializationException {
    ...
    for (int i = 0; i < TEST_LIMIT; i++) {
      executeDLQMessageCycle(String.format("key_%d", i), dataSourceID);
  ...
  private void executeDLQMessageCycle(final String key, final String dlqDataSourceID) throws RouterException,
    InterruptedException, JMSException, DeserializationException {
    _log.info("executing DLQ message cycle with keySource =  " + dlqDataSourceID);
    ...
    final Record record = createTestRecord(dlqDataSourceID, key);
    ...
    final Record recordAccepted = XmlSerializationUtils.deserialize(byteArray);
    assertEquals("Record id read from DLQ is not expected one!", record.getId(), recordAccepted.getId());
    _log.info(String.format("Extracted record with datasourceID = \"%s\" ; id.toString()= \"%s\"",
      recordAccepted.getId(), recordAccepted.getId().toString()));
  }
  ...
  protected Record createTestRecord(final String source, final String key) {
    ...
    final Record record = dataFactory.createRecord();
    record.setId(key);
    record.setSource(source);
    final AnyMap metadata = record.getMetadata();

    final AnySeq attribute = dataFactory.createAnySeq();
    attribute.add(dataFactory.createStringValue("my first value"));
    attribute.add(dataFactory.createLongValue(42));
    attribute.add(dataFactory.createDoubleValue(Math.PI));
    attribute.add(dataFactory.createBooleanValue(Boolean.TRUE));
