...
public class ListenerWorker extends AbstractLoggedComponent implements Runnable {
  ...
  private Record parseMessage(final Message message, final Properties messageProperties) throws ListenerException {
    ...
    try {
      ...
      final Record record = XmlSerializationUtils.deserialize(byteArray);
      ...
      final String operationStr = messageProperties.getProperty(JMSMessageProperties.PROPERTY_OPERATION);
      // calculate operation
      Operation operation;
      try {
      ...
      if (_log.isDebugEnabled()) {
        _log.debug(msg(String.format("processMessage: Operation=%s; %s", operation.toString(),
          XmlSerializationUtils.serialize2string(record))));
      }
      if (RecordLifecycleLogHelper.isRecordStateLogEnabled()) {
        RecordLifecycleLogHelper.logRecordState(
          "Record is processed by Listener with rule: [" + _ruleConfig.getName() + "] and operation ["
            + operation.name() + "]", record.getId());
      }
      return record;
    } catch (final Exception e) {
