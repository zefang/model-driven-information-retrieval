...
public abstract class AbstractTask<ConfigType> extends AbstractLoggedComponent implements
   ...
   * 
   */
  public String[] execute(TaskExecutionEnv env, ConfigType config, Map<String, Properties> idPropertyMap)
    ...
    try {
      ...
      final String[] ids = executeInternal(env, config, idPropertyMap);
      if (_log.isDebugEnabled()) {
   ...
   *           the exception
   */
  protected abstract String[] executeInternal(final TaskExecutionEnv env, final ConfigType config,
    Map<String, Properties> idPropertyMap) throws Exception;
}
