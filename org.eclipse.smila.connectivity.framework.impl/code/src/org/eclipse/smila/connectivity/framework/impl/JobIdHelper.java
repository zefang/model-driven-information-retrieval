...
public final class JobIdHelper {
   ...
   */
  public static void setJobIdAttribute(final Record record, final State state) {
    if (state != null) {
      setJobIdAttribute(record, state.getJobId());
    }
  }
   ...
   *          the id of the job
   */
  public static void setJobIdAttribute(final Record record, final String jobId) {
    if (record != null && record.getMetadata() != null && jobId != null && jobId.trim().length() > 0) {
      record.getMetadata().put(ConnectivityManager.ATTRIBUTE_JOB_ID, record.getFactory().createStringValue(jobId));
    }
  }
}
