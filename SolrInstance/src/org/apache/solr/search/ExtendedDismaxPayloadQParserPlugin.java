...
import org.apache.solr.search.customQueryParser;
 ...
 */
public class ExtendedDismaxPayloadQParserPlugin extends QParserPlugin {
  ...
  public QParser createParser(String qstr, SolrParams localParams, SolrParams params, SolrQueryRequest req) {
    return new ExtendedDismaxPayloadQParser(qstr, localParams, params, req);
  }
}


class ExtendedDismaxPayloadQParser extends QParser {
  ...
  public static final java.lang.String WPF = "wpf";
  ...
  public ExtendedDismaxPayloadQParser(String qstr, SolrParams localParams, SolrParams params, SolrQueryRequest req) {
  ...
  private HashMap<Float,Float> payloadWeights;
  ...
  public Query parse() throws ParseException {
    ...
    payloadWeights = new HashMap<Float,Float>();
    String[] weights = solrParams.getParams(ExtendedDismaxPayloadQParser.WPF);
    
    if(weights != null){    
    	for(String w : weights){    	
	    	String[] wv = w.split(":");    	    	
	    	try {								
	    		Float key = new Float(wv[0]);
				Float value = new Float(wv[1]);			
				payloadWeights.put(key, value);
			} catch (NumberFormatException e) {
				throw new SolrException( SolrException.ErrorCode.BAD_REQUEST, 
						"Error in parsing " + ExtendedDismaxPayloadQParser.WPF + " parameter. Got:" + wv.toString() );			
			}    	
    	}
    }
    ...
    if( userQuery == null || userQuery.length() < 1 ) {
      ...
      ExtendedSolrPayloadQueryParser up =
        new ExtendedSolrPayloadQueryParser(this, IMPOSSIBLE_FIELD_NAME);
      
      up.setPayloadWeights(payloadWeights);
      ...
      if (phraseFields.size() > 0 || phraseFields3.size() > 0) {
        ...
        if (normalClauses.size() >= 2 && pf.size() > 0) {
          ...
          ExtendedSolrPayloadQueryParser pp =
                  new ExtendedSolrPayloadQueryParser(this, IMPOSSIBLE_FIELD_NAME);
   ...
   */
  class ExtendedSolrPayloadQueryParser extends customQueryParser {
    ...
    protected HashMap<Float,Float> payWeights = new HashMap<Float,Float>(); 

    public ExtendedSolrPayloadQueryParser(QParser parser, String defaultField) {
    ...
    public void setPayloadWeights(HashMap<Float,Float> payWeights) {
    ...
    protected Query getBooleanQuery(List clauses, boolean disableCoord) throws ParseException {
      ...
      if (q != null) {
        q = new QueryUtils().makeQueryable(q);
    ...
    private Query getQuery() throws ParseException {
      try {
        ...
        switch (type) {
          ...
          case PHRASE:
            Query query = super.getFieldQuery(field, val, payWeights, "avg");
